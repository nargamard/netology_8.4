...
- hosts: clickhouse_cluster
  ...
  vars:
    ...
    # TLS encryption for the native TCP protocol (needs `clickhouse-role-client --secure`)
    ...
    clickhouse_ssl_server:
      certificate_file: "/etc/clickhouse-role-server/server.crt"
      private_key_file: "/etc/clickhouse-role-server/server.key"
      dh_params_file: "/etc/clickhouse-role-server/dhparam.pem"
